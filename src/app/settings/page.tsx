...
export default function SettingsPage() {
    ...
    const handleDragEndQuickAccess = async (event: DragEndEvent) => {
        ...
        if (active.id !== over?.id) {
            const currentOrder = profile?.preferences?.quickAccessOrder || [
            ...
            // Even if index is -1 (not found), arrayMove handles it gracefully or we should ensure all IDs are in order
            // To be safe, let's reconstruct if missing
            ...
            if (oldIndex === -1 || newIndex === -1) {
                ...
                newOrder = allIds;
                // Retry move
                const oldIdx = newOrder.indexOf(active.id as string);
                const newIdx = newOrder.indexOf(over?.id as string);
