...
export const useChatStore = create<ChatState>((set, get) => ({
    ...
    // We'll track if the user has read the thread. 
    // Real unread count requires joining with messages or a view.
    // For now, we will mark as unread if last_read_at is missing for a thread that is not new?
    // Actually, we'll just store the last_read_at in the thread object for the UI to decide.
    const participantMap = new Map(participants?.map(p => [p.thread_id, p]))

    // We really need 'last_message_at' from the DB to be useful.
    // Assuming the schema update included it or we rely on 'created_at' if it's the only timestamp.
    // Let's assume for now we don't have it and just use 'unreadCount' as a placeholder.
}
